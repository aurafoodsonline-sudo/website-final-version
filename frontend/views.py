...
from shop.models import Bundle as ShopBundle, Category as ShopCategory, Product as ShopProduct
...
def operations_console(request):
    ...
    # ── Storefront catalog (Products / Categories / Bundles) ────────────────
    # Migrated here from the CMS admin dashboard (templates/admin/dashboard.html).
    # These are shop.Product / shop.Category / shop.Bundle records — the public
    # storefront catalog — not erp.Product (see "Products by type" above, which
    # is the ERP's own internal inventory SKU master). Gated by the same
    # "admin.configure" permission the CMS dashboard used for can_catalog.
    can_catalog_manage = has_erp_permission(user, "admin.configure")
    shop_categories = list(
        ShopCategory.objects.order_by("sort_order", "name")
    ) if can_catalog_manage else []
    shop_products = list(
        ShopProduct.objects.select_related("category").order_by("-featured", "id")
    ) if can_catalog_manage else []
    shop_category_product_counts = {}
    for shop_product in shop_products:
        if shop_product.category_id:
            shop_category_product_counts[shop_product.category_id] = (
                shop_category_product_counts.get(shop_product.category_id, 0) + 1
            )
    shop_bundles = list(ShopBundle.objects.order_by("id")) if can_catalog_manage else []
    ...
    context = {
        ...
        "shop_category_product_counts": shop_category_product_counts,
        "shop_bundles": shop_bundles,
        "can": can,
    }

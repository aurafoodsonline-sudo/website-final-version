...
# NOTE: must stay readable by JavaScript (Django's own default) — every AJAX call on this
# site (star rating, cart quote, admin dashboard add/edit forms, ERP storefront-catalog
# forms) reads this cookie client-side to set the X-CSRFToken header. Setting this to True
# breaks all of those flows with a 403, since the token becomes invisible to document.cookie.
# The session cookie (the one that actually matters for hijacking) is separately protected
# by SESSION_COOKIE_HTTPONLY above, which is unaffected by this setting.
